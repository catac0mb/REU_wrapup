...
def train():
    ...
    model, log = train_model(model, optimizer, scheduler=None, dataloaders=dataloaders, dataset_sizes=dataset_sizes, device=DEVICE, 
                             loss_type=args.loss_type, num_epochs=args.epochs, save_path=save_path)
    ...
    df['epoch'] = log['epoch']
    df['training_loss'] = log['training_loss']
    df['training_IoU'] = log['training_IoU']
    df['val_loss'] = log['val_loss']
    df['val_IoU'] = log['val_IoU']
    train_log_save_filename = os.path.join(save_path, 'training_log.csv')
    df.to_csv(train_log_save_filename, columns=['epoch','training_loss','training_IoU', 'val_loss', 'val_IoU'], header=True,index=False,encoding='utf-8')
