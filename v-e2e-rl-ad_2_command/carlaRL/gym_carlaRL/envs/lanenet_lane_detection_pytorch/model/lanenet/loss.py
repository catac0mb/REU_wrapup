    ...
    def _discriminative_loss(self, embedding, seg_gt):
        ...
        for b in range(batch_size):
            ...
            for lane_idx in labels:
                ...
                # print(seg_mask_i.shape, embedding_b.shape)

                if not seg_mask_i.any():
                    continue
                
                # print(f'embedding_b: {embedding_b.shape}, seg_mask_i: {seg_mask_i.shape}')
                ...
                # mean_i = torch.sum(embedding_i) / torch.sum(seg_mask_i)
                # centroid_mean.append(mean_i)
                # ---------- var_loss -------------
                norms = torch.norm(embedding_i - mean_i.reshape(-1, 1), dim=0)
                
                var_loss = var_loss + torch.sum(F.relu(norms - self.delta_var) ** 2) / torch.sum(seg_mask_i) / num_lanes
            centroid_mean = torch.stack(centroid_mean)  # (n_lane, embed_dim)

            if num_lanes > 1:
                centroid_mean1 = centroid_mean.reshape(-1, 1, embed_dim)
                centroid_mean2 = centroid_mean.reshape(1, -1, embed_dim)
                # print(f'centroid_mean1: {centroid_mean1}, centroid_mean2: {centroid_mean2}')
        ...
        print(f'var_loss: {var_loss}, dist_loss: {dist_loss}')
