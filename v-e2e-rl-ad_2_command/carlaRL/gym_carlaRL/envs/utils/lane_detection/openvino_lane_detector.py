...
class OpenVINOLaneDetector():
    def __init__(self, cam_geom=CameraGeometry(), 
                 model_path='gym_carlaRL/envs/utils/converted_model/lane_model.xml', 
                 device="CPU") -> None:
        self.cg = cam_geom
    ...
    def detect_and_fit(self, img_array):
        img, left, right = self.detect(img_array)
        left_poly = self.fit_poly(left)
        right_poly = self.fit_poly(right)
        return left_poly, right_poly, img
    ...
    def fit_poly(self, probs):
        ...
        if not np.any(mask):
            ...
            default_coeffs = [0]
            return np.poly1d(default_coeffs)
